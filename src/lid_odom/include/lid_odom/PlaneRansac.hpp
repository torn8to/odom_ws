...
namespace cloud{
    inline Eigen::Vector4d PlaneRansac(std::vector<Eigen::Vector3d> points,
     ...
     int maxIteration = 1000){
        ...
        for(int i = 0, i < maxIterations, i++){
            ...
            double k = -crossABNorm.dot(out[1]);
            ...
            for (const auto& pt : points){
                distance_point = (plane_params[0]*pt[0] + plane_params[1] *pt[1] +
                                    plane_params[2]*pt[2]+plane_params[3])/crossABNorm.norm()
                if (abs(distance_point) < thresh){
                    inliers.emplace_back(pt);
                }
            }
            if(inliers.size() > bestInliers){
                bestEquation = paramsOfPlane;
    ...
    inline std::vector<uint> inlierIndidciesPlane(std::vector<Eigen::Vector3d &points,
    Eigen::Vector4d &paramsOfPlane, double threshold = 0.05){
